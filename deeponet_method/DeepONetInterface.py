...
class DeepONetMethod(SimulationMethod):
    ...
    def run_simulation(self, json_file_path):
        self._deeponet_method(json_file_path)
    ...
    def _convert_relative_to_absolute_paths(
        ...
        data["dg_setup"]["output_path"] = self._resolve_path(
            data["dg_setup"]["output_path"], dirname
        )
        data["deeponet_train_setup"]["input_dir"] = self._resolve_path(
            data["deeponet_train_setup"]["input_dir"], dirname
        )
        data["deeponet_train_setup"]["output_dir"] = self._resolve_path(
    ...
    def _prepare_dg_json(self, json_file_path: str) -> str:
        """
    ...
    def _run_dg_simulation(self, json_file_path: str | Path) -> None:
        """
    ...
    def _process_source_data(
            self,
            results_dg: dict, source_position: np.ndarray) -> tuple:
        """
    ...
    def _read_wav_impulse_response(self, wav_path: str) -> list[float]:
        """
    ...
    def _write_results_json(self,
        ...
        for wav_file in pred_wav_files:
            ...
            try:
                source_pos, receiver_pos = self._parse_receiver_position_from_filename(filename)
                ir_data = self._read_wav_impulse_response(wav_file)
    ...
    def _deeponet_method(self, json_file_path: str | Path, output_json_path: str | Path = None):
        """
        ...
        settings = self._convert_relative_to_absolute_paths(json_file_path, dirname)
        ...
        dg_json = self._prepare_dg_json(json_file_path)
        self._run_dg_simulation(dg_json)
        ...
        mesh, pressures, time_steps, source_positions, results_dg = (
            self._load_and_process_dg_results(output_path, output_filename, file_format)
        )
        ...
        for i, source_position in enumerate(source_positions):
            ...
            umesh, upressures, ushape = self._process_source_data(results_dg, source_position)
        ...
        self._write_results_json(json_file_path, settings, output_json_path)
...
if __name__ == "__main__":
    ...
    deeponet_method = DeepONetMethod()
    # Run the complete DeepONet pipeline (includes DG simulation and results writing)
    deeponet_method.run_simulation(json_tmp_file, results_json_path)
